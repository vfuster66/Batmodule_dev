      ...
      <div class="flex items-center justify-between">
        <div class="flex gap-3">
          ...
          >
            ...
            {{
              isProcessingReminders ? "Traitement..." : "Traiter les rappels"
            }}
          </button>
        </div>
        <button
                    ...
                    >
                      ...
                      >
                        <path
                          ...
                          stroke-linejoin="round"
                          stroke-width="2"
                          d="M5 8l6 6 6-6M5 8h14M5 8v10a2 2 0 002 2h10a2 2 0 002-2V8"
                        />
                      </svg>
                    </button>
                    <button
...
const services = ref([]);
const isProcessingReminders = ref(false);
...
const handleSearch = async () => {
  try {
    await invoicesStore.fetchInvoices({ search: searchTerm.value, page: 1 });
  } catch (error) {
...
const handleStatusFilter = async () => {
  try {
    await invoicesStore.fetchInvoices({ status: statusFilter.value, page: 1 });
  } catch (error) {
    console.error("Erreur filtrage factures:", error);
    toast.error("Le filtrage des factures a échoué");
  }
};
...
const searchClients = async () => {
  ...
  try {
    const response = await api.get(
      `/clients?search=${encodeURIComponent(clientSearch.value)}&limit=10`,
    );
...
// onMounted supprimé car dupliqué
...
const sendInvoiceByEmail = async (invoice) => {
  try {
    await invoicesStore.sendByEmail(invoice.id);
...
const getPaymentStatusLabel = (invoice) => {
  ...
  // Pour les factures d'acompte et de solde, utiliser le total du devis
  let totalTtc = invoice.totalTtc || 0;
  if (invoice.invoiceType === "acompte" || invoice.invoiceType === "solde") {
    totalTtc = invoice.quoteTotalTtc || invoice.totalTtc || 0;
  }
...
const getInvoiceAmount = (invoice) => {
  // Pour les factures d'acompte et de solde, afficher le total du devis
  if (invoice.invoiceType === "acompte" || invoice.invoiceType === "solde") {
    const amount = invoice.quoteTotalTtc || 0;
    return amount;
  }

  // Pour les autres factures, afficher le total de la facture
  const amount = invoice.totalTtc || 0;
  return amount;
};

const getPaidAmount = (invoice) => {
  // Pour les factures d'acompte, afficher le montant de l'acompte
  if (invoice.invoiceType === "acompte") {
    const amount = invoice.totalTtc || 0;
    return amount;
  }

  // Pour les autres factures, afficher le montant payé
...
const getRemainingAmount = (invoice) => {
  ...
  // Pour les factures d'acompte, calculer : total du devis - montant de l'acompte
  if (invoice.invoiceType === "acompte") {
    const quoteTotal = invoice.quoteTotalTtc || 0;
    const acompteAmount = invoice.totalTtc || 0;
    const remaining = Math.max(0, quoteTotal - acompteAmount);
    return remaining;
  }

  // Pour les factures de solde, utiliser le montant payé normal
  if (invoice.invoiceType === "solde") {
    const quoteTotal = invoice.quoteTotalTtc || 0;
    const totalPaid = invoice.paidAmount || 0;
  ...
  // Pour les autres factures, utiliser le total de la facture
...
// Fonction pour traiter les rappels
const processReminders = async () => {
  try {
    isProcessingReminders.value = true;
    await api.post("/invoices/reminders/process");
    toast.success("Rappels traités avec succès");
    await invoicesStore.fetchInvoices(); // Rafraîchir la liste
  } catch (error) {
    console.error("Erreur lors du traitement des rappels:", error);
    toast.error("Erreur lors du traitement des rappels");
  } finally {
    isProcessingReminders.value = false;
  }
};
