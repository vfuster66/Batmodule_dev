...
export const useCompanySettingsStore = defineStore('companySettings', () => {
  ...
  const isCompliant = computed(() => {
    return complianceScore.value >= 80
  })

  const missingFields = computed(() => {
    if (complianceScore.value >= 80) return []
  ...
  const convertFileToBase64 = (file) => {
    return new Promise((resolve, reject) => {
      ...
      reader.onload = () => {
        ...
        // Optimiser les fichiers SVG volumineux
        if (file.type === 'image/svg+xml' && result.length > 100000) {
          // Plus de 100KB
          console.warn(
            '⚠️ Fichier SVG volumineux détecté, optimisation recommandée'
          )
          // Pour l'instant, on accepte mais on pourrait ajouter une compression ici
        }
  ...
  // Uploader un cachet et le sauvegarder côté API
  const uploadStamp = async (file) => {
    try {
      ...
      const base64 = await convertFileToBase64(file)
      await updateSetting('stamp_base64', base64)
      return base64
    } catch (err) {
      console.error('Erreur uploadStamp:', err)
      throw err
    }
  }
