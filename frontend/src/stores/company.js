  ...
  actions: {
    ...
    async fetchSettings() {
      ...
      try {
        ...
        console.log("🏢 Paramètres reçus du serveur:", serverSettings);
        ...
        if (serverSettings) {
          ...
          console.log(
            "🖼️ Logo après mise à jour:",
            this.settings.logo_base64 ? "Présent" : "Absent",
          );
        }
      } catch (error) {
        this.error =
    ...
    async uploadStamp(file) {
      try {
        ...
        const base64 = await this.convertFileToBase64(file);
        console.log(
          "✅ Cachet converti en base64, taille:",
          base64.length,
          "caractères",
        );

        await this.updateSetting("stamp_base64", base64);
        console.log("✅ Cachet sauvegardé avec succès");
