...
const pool = new Pool({
    connectionString: process.env.DATABASE_URL || 'postgresql://batmodule:batmodule123@localhost:5432/batmodule',
...
async function query(text, params, userId = null) {
    ...
    try {
        ...
        if (duration > 100) {
            console.log('⚠️ Requête lente:', { text: text.substring(0, 100) + '...', duration, rows: result.rowCount, userId: userId ? 'set' : 'none' });
        }

        return result;
    } catch (error) {
        console.error('❌ Erreur de requête:', { text: text.substring(0, 100) + '...', error: error.message, userId: userId ? 'set' : 'none' });
        throw error;
    } finally {
