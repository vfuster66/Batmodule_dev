...
class AccountingExportService {
    ...
    async generateFECExport(userId, startDate, endDate) {
        try {
            ...
            const entries = await this.getAccountingEntries(userId, startDate, endDate);

            // Générer le fichier FEC
            const fecContent = this.generateFECContent(company, entries, startDate, endDate);
    ...
    generateFECContent(company, entries, startDate, endDate) {
        ...
        lines.push(`#PERIODE:${this.formatDateForFEC(startDate)}:${this.formatDateForFEC(endDate)}`);
