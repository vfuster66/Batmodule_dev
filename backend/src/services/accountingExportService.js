...
class AccountingExportService {
  ...
  async getAccountingEntries(userId, startDate, endDate) {
    ...
    // Récupérer les factures avec leurs items pour calculer la TVA réelle
    const invoicesResult = await query(
      `SELECT 
                'VENTE' as journal_code,
                'VTE' as journal_lib,
                i.invoice_number as piece_num,
                i.invoice_date as piece_date,
                'Facture ' || i.invoice_number as piece_lib,
                '411' as compte_num,
                COALESCE(c.company_name, c.first_name || ' ' || c.last_name) as compte_lib,
                'D' as sens,
                i.total_ttc as debit,
                0 as credit,
                'Facture ' || i.invoice_number as ecriture_lib,
                'EUR' as currency_code,
                NULL as currency_rate,
                i.created_at as ecriture_date,
                'F' as valid_date,
                'F' as ecriture_let,
                'F' as date_let,
                'F' as ecriture_annee,
                ...
                i.total_vat,
                i.total_ttc,
                -- Recalculer les montants à partir des items pour vérification
                COALESCE(SUM(ii.total_ht), 0) as calculated_subtotal_ht,
                COALESCE(SUM(ii.total_ht * ii.vat_rate / 100), 0) as calculated_total_vat,
                COALESCE(SUM(ii.total_ttc), 0) as calculated_total_ttc
             FROM invoices i
    ...
    for (const invoice of invoicesResult.rows) {
      ...
      if (invoice.debit > 0) {
        // Utiliser les montants recalculés si les valeurs stockées semblent incohérentes
        const storedVat = parseFloat(invoice.total_vat) || 0
        const calculatedVat = parseFloat(invoice.calculated_total_vat) || 0
        const storedSubtotal = parseFloat(invoice.subtotal_ht) || 0
        const calculatedSubtotal =
          parseFloat(invoice.calculated_subtotal_ht) || 0

        // Utiliser les montants recalculés si la différence est significative (> 0.01€)
        const vatAmount =
          Math.abs(storedVat - calculatedVat) > 0.01 ? calculatedVat : storedVat
