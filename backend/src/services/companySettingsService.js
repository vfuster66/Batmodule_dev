...
class CompanySettingsService {
    ...
    try {
      ...
      const result = await query(
        ...
        [
          ...
          defaultSettings.default_vat_rate,
          defaultSettings.currency,
          defaultSettings.payment_terms,
          defaultSettings.quote_prefix,
          defaultSettings.invoice_prefix,
          defaultSettings.show_vat,
          defaultSettings.show_logo_on_documents,
  ...
  validateIBAN(iban) {
    ...
    // Format basique pour la France - accepte les lettres et chiffres
    const ibanRegex =
      /^FR\d{2}\s?[\dA-Z]{4}\s?[\dA-Z]{4}\s?[\dA-Z]{4}\s?[\dA-Z]{4}\s?[\dA-Z]{4}\s?[\dA-Z]{2}$/
    return ibanRegex.test(iban) && iban.length >= 27 // Minimum 27 caractères pour un IBAN français
  }
