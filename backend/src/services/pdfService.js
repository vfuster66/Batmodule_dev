...
class PDFService {
  ...
  async generateQuotePDF(quote, companySettings) {
    return this.withPage(async (page) => {
      ...
      const addr = [
        ...
        .filter(Boolean)
        .join(' ')
      const cityLine = [companySettings?.postal_code, companySettings?.city]
        .filter(Boolean)
        .join(' ')
      return page.pdf({
        format: 'A4',
        ...
        headerTemplate: `
                ...
                `,
        footerTemplate: await this.buildFooterTemplate(
          companySettings,
          quote.userId
        ),
      })
  ...
  async buildFooterTemplate(companySettings = {}, userId = null) {
    ...
    const tva = lc(
      companySettings.tva_intracommunautaire || companySettings.vat_number
    )
    ...
    const assurance = lc(
      companySettings.assurance_rc || companySettings.insurance_company
    )
    ...
    if (userId) {
      try {
        ...
        activeInsurances = activeInsurances.filter(
          (ins) => ins.is_active && new Date(ins.end_date) > new Date()
        )
        activeCertifications = activeCertifications.filter(
          (cert) => cert.is_active && new Date(cert.end_date) > new Date()
        )
      } catch (error) {
        console.error(
          'Erreur lors de la récupération des assurances/certifications:',
          error
        )
      }
    }
    ...
    const legalLines = [
      ...
      assurance
        ? `Assurance RC Pro: ${assurance}${police ? ' — Police: ' + police : ''} `
        : '',
      [phone, email, website ? website.replace(/^https?:\/\//, '') : '']
        .filter(Boolean)
        .join(' — '),
    ]
      .filter(Boolean)
    ...
    const certificationLines = activeCertifications.map((cert) => {
      ...
      const typeName =
        typeNames[cert.certification_type] || cert.certification_type
...
<body>
    <div class='header-section recipient-row'>
        ...
        <div class='client-card client-info'>
            ...
            ${
              quote.siteAddress &&
              (quote.siteAddress.addressLine1 || quote.siteAddress.addressLine2)
            ...
            ${(() => {
              ...
              if (Array.isArray(quote.sections) && quote.sections.length) {
                for (let i = 0; i < quote.sections.length; i++) {
                  ...
                  rows.push(
                    `<tr><td colspan='7' style='font-weight:bold;color:#111;border-left:3px solid ${primaryColor};padding-left:8px;background:#f8f9fa;'>${sec.title || ''}</td></tr>`
                  )
                  if (sec.description)
                    rows.push(
                      `<tr><td colspan='7' style='color:#555;font-size:10px;padding-left:8px;'>${sec.description}</td></tr>`
                    )
...
<body>
    <div class='header-section recipient-row'>
        ...
        <div class='client-card client-info'>
            ...
            ${
              invoice.siteAddress &&
              (invoice.siteAddress.addressLine1 ||
                invoice.siteAddress.addressLine2)
    ...
    <div class='totals-section'>
        <table class='totals-table'>
            ...
            ${Array.from(vatMap.entries())
              ...
              .map(
                ([rate, amt]) =>
                  `<tr><td>TVA ${rate}%:</td><td class='text-right'>${formatCurrency(amt)}</td></tr>`
              )
              .join('')}
