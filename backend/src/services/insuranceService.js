        ...
        try {
            ...
            const result = await query(
                `INSERT INTO insurance_certificates 
                 ...
                 RETURNING *`,
                [
                    userId, certificateType, certificateNumber, insuranceCompany,
                    policyNumber, startDate, endDate, coverageAmount, deductible,
                    notes, documentUrl
                ]
            );
        ...
        try {
            ...
            result.rows.forEach(row => {
                ...
                if (row.expired_count > 0) {
                    compliance.warnings.push(`${row.certificate_type}: ${row.expired_count} assurance(s) expirée(s)`);
                }

                if (row.expiring_soon_count > 0) {
                    compliance.warnings.push(`${row.certificate_type}: ${row.expiring_soon_count} assurance(s) expirant bientôt`);
                }
            });
        ...
        try {
            ...
            result.rows.forEach(insurance => {
                ...
                if (insurance.end_date <= new Date()) {
                    summary.expired++;
                } else if (insurance.end_date <= new Date(Date.now() + 30 * 24 * 60 * 60 * 1000)) {
