class CalculationService {
    ...
    calculateItemTotals(item) {
        ...
        const discountPercent = Math.max(0, Math.min(100, parseFloat(item.discountPercent ?? 0))) || 0;
        const markupPercent = Math.max(0, Math.min(100, parseFloat(item.markupPercent ?? item.surchargePercent ?? 0))) || 0;

        // Appliquer remise et/ou majoration sur le prix unitaire HT
        const unitPriceNetHt = unitPriceHt * (1 - discountPercent / 100) * (1 + markupPercent / 100);
    ...
    calculateTotals(items) {
        ...
        const totalTtc = calculatedItems.reduce((sum, item) => sum + item.totalTtc, 0);
    ...
    validateItem(item) {
        ...
        if (item.vatRate === undefined || item.vatRate === null || parseFloat(item.vatRate) < 0) {
        ...
        if (item.markupPercent !== undefined || item.surchargePercent !== undefined) {
