...
class AdvanceInvoiceService {
    ...
    async createAdvanceInvoice(params) {
        ...
        return await transaction(async (client) => {
            ...
            const vatRate = 20.00; // Taux par défaut, peut être paramétrable
            ...
            const invoiceResult = await client.query(
                ...
                [
                    userId, clientId, quoteId, invoiceNumber, title, description,
                    advanceAmountHt, vatAmount, advanceAmount, dueDate, notes, advanceAmount, purchaseOrderNumber
                ]
            );
    ...
    async createFinalInvoice(params) {
        ...
        return await transaction(async (client) => {
            ...
            const advanceAmount = advanceInvoice.advance_amount || advanceInvoice.total_ttc;
            ...
            if (remainingAmount <= 0) {
                throw new Error('Le montant de l\'acompte est supérieur ou égal au montant total du projet');
            }
            ...
            const invoiceResult = await client.query(
                ...
                [
                    userId, clientId, quoteId, invoiceNumber, title, description,
                    calculations.subtotalHt, calculations.totalVat, remainingAmount,
                    dueDate, notes, parentInvoiceId, advanceAmount, purchaseOrderNumber
                ]
            );
            ...
            for (let i = 0; i < calculations.items.length; i++) {
                ...
                const itemResult = await client.query(
                    ...
                    RETURNING id, service_id, description, quantity, unit_price_ht, 
                             unit_price_ttc, vat_rate, total_ht, total_ttc, sort_order, section_id`,
                    [
                        invoice.id, item.serviceId, item.description, item.quantity,
                        item.unitPriceHt, item.unitPriceTtc, item.vatRate, item.totalHt,
                        item.totalTtc, item.sortOrder || i, item.sectionId || null
                    ]
                );
    ...
    async getPaymentStatus(userId, parentInvoiceId) {
        const relatedInvoices = await this.getRelatedInvoices(userId, parentInvoiceId);
