...
class BTPValidationService {
    ...
    generateWarnings(invoiceData) {
        ...
        if (invoiceData.reverseChargeBtp && !invoiceData.clientVatNumber) {
            warnings.push('Autoliquidation BTP activée mais numéro de TVA client manquant');
        }

        // Avertissement pour TVA réduite
        if (invoiceData.reducedVatApplied) {
            if (invoiceData.propertyType === 'commercial') {
                warnings.push('TVA réduite généralement non applicable aux locaux commerciaux');
            }
            if (invoiceData.propertyAgeYears < 2 && invoiceData.reducedVatRate === 10) {
    ...
    async calculateApplicableVATRate(invoiceData) {
        ...
        if (invoiceData.reverseChargeBtp) {
            ...
            if (btpValidation.is_valid) {
                applicableRate = 0.0;
                justification = 'TVA en sus, autoliquidation par le client (art. 283-2 CGI)';
        ...
        if (invoiceData.reducedVatApplied && !invoiceData.reverseChargeBtp) {
            ...
            if (vatValidation.is_valid) {
                ...
                if (invoiceData.reducedVatRate === 10) {
                    justification = 'TVA 10% - Travaux de rénovation dans des locaux à usage d\'habitation achevés depuis plus de deux ans';
                    ...
                    justification = 'TVA 5,5% - Travaux d\'amélioration de la qualité énergétique des locaux à usage d\'habitation';
                    conditions.push('Logement d\'habitation');
        ...
        return {
            ...
            isReducedRate: invoiceData.reducedVatApplied && !invoiceData.reverseChargeBtp
        };
    }
