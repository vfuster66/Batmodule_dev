...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la récupération des certifications' })
  }
})
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la récupération de la certification' })
  }
})
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la création de la certification' })
  }
})
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la mise à jour de la certification' })
  }
})
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params
    const deleted = await certificationService.deleteCertification(
      id,
      req.user.userId
    )
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la suppression de la certification' })
  }
})
...
router.get('/alerts/expiring', authenticateToken, async (req, res) => {
  try {
    ...
    const expiringCertifications =
      await certificationService.getExpiringCertifications(
        req.user.userId,
        parseInt(days)
      )
    ...
    res.status(500).json({
      error: 'Erreur lors de la récupération des certifications expirantes',
    })
  }
})
...
router.get('/compliance/check', authenticateToken, async (req, res) => {
  try {
    const compliance = await certificationService.checkCertificationCompliance(
      req.user.userId
    )
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la vérification de conformité' })
  }
})
...
router.get('/types/info', authenticateToken, async (req, res) => {
  try {
    const certificationTypes =
      await certificationService.getCertificationTypes()
    ...
    console.error(
      'Erreur lors de la récupération des types de certifications:',
      error
    )
    res.status(500).json({
      error: 'Erreur lors de la récupération des types de certifications',
    })
  }
})
