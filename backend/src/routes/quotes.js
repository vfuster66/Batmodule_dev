...
async function ensureQuoteSchema() {
    try {
        ...
        await query('ALTER TABLE quote_items ADD COLUMN IF NOT EXISTS section_id UUID REFERENCES quote_sections(id) ON DELETE SET NULL');
        await query('ALTER TABLE quote_items ADD COLUMN IF NOT EXISTS unit VARCHAR(50)');
        await query('ALTER TABLE quote_items ADD COLUMN IF NOT EXISTS discount_percent DECIMAL(5,2) DEFAULT 0');
        await query('ALTER TABLE quote_items ADD COLUMN IF NOT EXISTS markup_percent DECIMAL(5,2) DEFAULT 0');

        // Adresse de chantier (si pas de migrations en place)
        await query("ALTER TABLE quotes ADD COLUMN IF NOT EXISTS site_address_line1 TEXT");
        await query("ALTER TABLE quotes ADD COLUMN IF NOT EXISTS site_address_line2 TEXT");
        await query("ALTER TABLE quotes ADD COLUMN IF NOT EXISTS site_postal_code TEXT");
        ...
        await query("ALTER TABLE quotes ADD COLUMN IF NOT EXISTS site_same_as_billing BOOLEAN DEFAULT false");
    } catch (e) {
...
router.post('/', authenticateToken, async (req, res, next) => {
    try {
        ...
        const { clientId, title, description, validUntil, notes, depositPercent, depositAmount,
            siteSameAsBilling, siteAddressLine1, siteAddressLine2, sitePostalCode, siteCity, siteCountry } = value;
        const hasSections = Array.isArray(value.sections) && value.sections.length > 0;
        const flatItems = hasSections ? value.sections.flatMap((s, idx) => (s.items || []).map(it => ({ ...it, sectionSortOrder: s.sortOrder ?? idx }))) : value.items;
        ...
        const result = await transaction(async (client) => {
            ...
            const quoteResult = await client.query(
                ...
                [req.user.userId, clientId, quoteNumber, title, description, 'draft', calculations.subtotalHt, calculations.totalVat, calculations.totalTtc, validUntil, notes, finalDepositAmount, false,
                siteSameAsBilling || false, siteAddressLine1 || null, siteAddressLine2 || null, sitePostalCode || null, siteCity || null, siteCountry || null]
            );
            ...
            if (hasSections) {
                ...
                for (let si = 0; si < value.sections.length; si++) {
                    ...
                    for (let i = 0; i < itemsCalc.items.length; i++) {
                        ...
                        const itemResult = await client.query(
                            ...
                            [quote.id, secRes.rows[0].id, item.serviceId, item.description, item.quantity, item.unitPriceHt, item.unitPriceTtc, item.vatRate, item.unit || null, item.discountPercent || 0, item.markupPercent || item.surchargePercent || 0, item.totalHt, item.totalTtc, item.sortOrder || i]
                        );
                ...
                for (let i = 0; i < calculations.items.length; i++) {
                    ...
                    const itemResult = await client.query(
                        ...
                        [quote.id, item.serviceId, item.description, item.quantity, item.unitPriceHt, item.unitPriceTtc, item.vatRate, item.unit || null, item.discountPercent || 0, item.markupPercent || item.surchargePercent || 0, item.totalHt, item.totalTtc, item.sortOrder || i]
                    );
                    quoteItems.push(itemResult.rows[0]);
                }
            }
        ...
        // Audit
        try { await logAudit({ userId: req.user.userId, entityType: 'quote', entityId: result.quote.id, action: 'create', metadata: { quoteNumber } }); } catch (_) { }
...
router.put('/:id/status', authenticateToken, async (req, res, next) => {
    try {
       ...
       RETURNING id, status, updated_at`,
        ...
        // Audit
        try { await logAudit({ userId: req.user.userId, entityType: 'quote', entityId: id, action: 'status', metadata: { status } }); } catch (_) { }
...
router.put('/:id', authenticateToken, async (req, res, next) => {
    try {
        ...
        if (current.rows[0].status !== 'draft') {
            return res.status(400).json({ error: 'Devis non modifiable (statut non draft)' });
        }
        ...
        const { title, description, validUntil, notes, siteSameAsBilling, siteAddressLine1, siteAddressLine2, sitePostalCode, siteCity, siteCountry, sections } = value;
        const hasSections = Array.isArray(sections) && sections.length > 0;
        const flatItems = hasSections ? sections.flatMap((s, idx) => (s.items || []).map(it => ({ ...it, sectionSortOrder: s.sortOrder ?? idx }))) : [];
        ...
        const quoteResult = await query(
            ...
            RETURNING *`,
            [title, description, validUntil, notes, siteSameAsBilling || false, siteAddressLine1 || null, siteAddressLine2 || null,
                sitePostalCode || null, siteCity || null, siteCountry || null, calculations.subtotalHt, calculations.totalVat, calculations.totalTtc, id, req.user.userId]
        );
            ...
            for (let si = 0; si < sections.length; si++) {
                ...
                if (section.items && section.items.length > 0) {
                    for (let ii = 0; ii < section.items.length; ii++) {
                        ...
                        const itemCalculations = calculationService.calculateItemTotals(item);
                        await query(
                            ...
                            [id, sectionId, item.description, item.quantity, item.unitPriceHt, item.unitPriceTtc || itemCalculations.unitPriceTtc,
                                item.vatRate, item.unit, itemCalculations.totalHt, itemCalculations.totalTtc, ii]
                        );
                    }
                }
            }
...
router.get('/:id/pdf', authenticateToken, async (req, res, next) => {
    try {
       ...
       ORDER BY sort_order, created_at`,
        ...
        // Générer le PDF
        const pdfBuffer = await pdfService.generateQuotePDF(quoteData, companySettings);

        // Définir les headers pour le téléchargement
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="devis-${quote.quote_number}.pdf"`);
...
// ====== ROUTES PUBLIC/EMAIL (après module.exports pour lisibilité dans ce patch) ======
    ...
    try {
        ...
        const quoteRes = await query(
            `SELECT q.id, q.user_id, q.client_id, q.quote_number, q.title, q.description, q.status,
             ...
             WHERE q.id = $1 AND q.user_id = $2`, [id, req.user.userId]);
        if (quoteRes.rows.length === 0) return res.status(404).json({ error: 'Devis non trouvé' });
        const q = quoteRes.rows[0];

        const itemsRes = await query(
            `SELECT id, section_id, service_id, description, quantity, unit_price_ht, unit_price_ttc, vat_rate, unit, discount_percent, markup_percent, total_ht, total_ttc, sort_order
             FROM quote_items WHERE quote_id = $1 ORDER BY sort_order, created_at`, [id]);
        const sectionsRes = await query(
            `SELECT id, title, description, sort_order FROM quote_sections WHERE quote_id = $1 ORDER BY sort_order, created_at`, [id]);
        const companyRes = await query(`SELECT * FROM company_settings WHERE user_id = $1`, [req.user.userId]);
        const company = companyRes.rows[0] || {};

        // Générer/Insérer un token public (30 jours)
        const token = crypto.randomBytes(24).toString('hex');
        const expiresAt = new Date(Date.now() + 30 * 24 * 60 * 60 * 1000);
        await query(`INSERT INTO quote_tokens (quote_id, token, purpose, expires_at) VALUES ($1, $2, 'public', $3)`, [id, token, expiresAt]);
        ...
        const quoteData = {
            ...
            items: itemsRes.rows.map(it => ({
                ...
                unit: it.unit,
                discountPercent: it.discount_percent ? parseFloat(it.discount_percent) : 0,
            ...
            sections: sectionsRes.rows.map(sec => ({ id: sec.id, title: sec.title, description: sec.description, sortOrder: sec.sort_order })),
        ...
        const mailSubject = subject || `Votre devis n° ${q.quote_number} – ${company.company_name || 'Fuster Peinture'}`;
        ...
        try {
            info = await transport.sendMail({
                ...
                subject: mailSubject,
                html,
                attachments: [{ filename: `devis-${q.quote_number}.pdf`, content: pdfBuffer }]
            });
        } catch (e) {
        ...
        await query(`INSERT INTO email_logs (user_id, quote_id, to_email, cc_email, subject, status, provider_id, error_message)
                     VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
            [req.user.userId, id, to, cc || null, mailSubject, info?.messageId ? 'sent' : 'queued', info?.messageId || null, info?.error || null]);

        // Historique + statut
        await query(`INSERT INTO quote_status_history (quote_id, event_type, metadata) VALUES ($1,'sent', $2)`, [id, JSON.stringify({ to, cc, providerId: info?.messageId || null })]);
        await query(`UPDATE quotes SET status = CASE WHEN status = 'draft' THEN 'sent' ELSE status END, updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [id]);

        res.json({ message: 'Devis envoyé', publicUrl, email: { messageId: info?.messageId || null } });
...
router.get('/:id/public', async (req, res, next) => {
    try {
        ...
        // Valider le token
        const tokRes = await query(`SELECT * FROM quote_tokens WHERE quote_id = $1 AND token = $2 AND purpose = 'public'`, [id, token]);
        if (tokRes.rows.length === 0) return res.status(403).send('Token invalide');
        const t = tokRes.rows[0];
        if (t.expires_at && new Date(t.expires_at).getTime() < Date.now()) return res.status(403).send('Token expiré');

        // Charger devis pour résumé
        const qRes = await query(`SELECT quote_number, title, subtotal_ht, total_vat, total_ttc, valid_until FROM quotes WHERE id = $1`, [id]);
        if (qRes.rows.length === 0) return res.status(404).send('Devis non trouvé');
        const q = qRes.rows[0];

        await query(`INSERT INTO quote_status_history (quote_id, event_type, ip, user_agent) VALUES ($1,'viewed',$2,$3)`, [id, req.ip, req.get('user-agent') || null]);
        ...
        <div class="box">
          <form method="POST" action="/api/quotes/${id}/accept">
            ...
            <input id="otp" name="otp" type="text" maxlength="6" placeholder="123456" pattern="\\\d{6}" required />
...
router.get('/:id/public.json', async (req, res, next) => {
    try {
        ...
        const tokRes = await query(`SELECT * FROM quote_tokens WHERE quote_id = $1 AND token = $2 AND purpose = 'public'`, [id, token]);
        if (tokRes.rows.length === 0) return res.status(403).json({ error: 'Token invalide' });
        const t = tokRes.rows[0];
        if (t.expires_at && new Date(t.expires_at).getTime() < Date.now()) return res.status(403).json({ error: 'Token expiré' });
...
router.post('/:id/accept', express.json(), express.urlencoded({ extended: true }), async (req, res, next) => {
    try {
        await ensureQuoteAuxSchemas();
        const { id } = req.params; const { token, otp } = req.body || {};
        if (!token) return res.status(400).send('Token manquant');
        const tokRes = await query(`SELECT * FROM quote_tokens WHERE quote_id = $1 AND token = $2 AND purpose = 'public'`, [id, token]);
        if (tokRes.rows.length === 0) return res.status(403).send('Token invalide');
        const t = tokRes.rows[0];
        ...
        if (!otp || !/^\d{6}$/.test(otp)) return res.status(400).send('Code de validation requis');
        const otpRes = await query(`SELECT * FROM quote_tokens WHERE quote_id = $1 AND purpose = 'otp' ORDER BY created_at DESC LIMIT 1`, [id]);
        if (otpRes.rows.length === 0) return res.status(400).send('Aucun code en cours, demandez un nouveau code.');
        const otpRow = otpRes.rows[0];
        if (otpRow.expires_at && new Date(otpRow.expires_at).getTime() < Date.now()) return res.status(400).send('Code expiré, demandez un nouveau code.');
        // token champ contient "salt:hash"
        const [salt, storedHash] = String(otpRow.token).split(':');
        const checkHash = crypto.createHash('sha256').update(`${salt}:${otp}`).digest('hex');
        if (checkHash !== storedHash) {
            // historiser l'échec et limiter les tentatives
            await query(`INSERT INTO quote_status_history (quote_id, event_type, metadata, ip, user_agent) VALUES ($1,'otp_fail',$2,$3,$4)`, [id, JSON.stringify({ reason: 'mismatch' }), req.ip, req.get('user-agent') || null]);
            const fails = await query(`SELECT COUNT(*) AS c FROM quote_status_history WHERE quote_id = $1 AND event_type = 'otp_fail' AND created_at > NOW() - INTERVAL '30 minutes'`, [id]);
            const c = parseInt(fails.rows[0].c || '0', 10);
            if (c >= 5) return res.status(429).send('Trop de tentatives. Demandez un nouveau code et réessayez plus tard.');
            return res.status(400).send('Code invalide.');
        }

        // Marquer OTP utilisé
        await query(`UPDATE quote_tokens SET used_at = CURRENT_TIMESTAMP WHERE id = $1`, [otpRow.id]);
        ...
        const quoteRes = await query(
            `SELECT q.id, q.user_id, q.client_id, q.quote_number, q.title, q.description, q.status,
                    ...
                    c.first_name, c.last_name, c.company_name, c.email, c.phone,
                    c.address_line1, c.address_line2, c.postal_code, c.city, c.country
             FROM quotes q JOIN clients c ON q.client_id = c.id WHERE q.id = $1`, [id]);
        const itemsRes = await query(`SELECT id, section_id, service_id, description, quantity, unit_price_ht, unit_price_ttc, vat_rate, unit, discount_percent, markup_percent, total_ht, total_ttc, sort_order FROM quote_items WHERE quote_id = $1 ORDER BY sort_order, created_at`, [id]);
        const sectionsRes = await query(`SELECT id, title, description, sort_order FROM quote_sections WHERE quote_id = $1 ORDER BY sort_order, created_at`, [id]);
        const companyRes = await query(`SELECT * FROM company_settings WHERE user_id = $1`, [quoteRes.rows[0].user_id]);
        const q = quoteRes.rows[0];
        const company = companyRes.rows[0] || {};
        const quoteData = {
            id: q.id,
        ...
        await query(`UPDATE quotes SET status = 'accepted', updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [id]);
        await query(`INSERT INTO quote_status_history (quote_id, event_type, ip, user_agent, metadata) VALUES ($1,'accepted',$2,$3,$4)`, [id, req.ip, req.get('user-agent') || null, JSON.stringify({ pdfSha256 })]);
        ...
        try {
            ...
            const html = `<p>Bonjour ${q.first_name || ''} ${q.last_name || ''},</p>
                        ...
                        <p>Cordialement,</p>`;
            await transport.sendMail({ from, to, subject, html, attachments: [{ filename: `devis-${q.quote_number}.pdf`, content: pdfBuffer }] });
            await query(`INSERT INTO email_logs (user_id, quote_id, to_email, subject, status) VALUES ($1,$2,$3,$4,'sent')`, [q.user_id, id, to, subject]);
        } catch (_) { /* ignore SMTP failures */ }

        res.send('<html><body style="font-family:Arial;padding:20px"><h2>Merci, devis accepté.</h2><p>Votre décision a été enregistrée.</p></body></html>');
    } catch (error) { next(error); }
});

// POST /api/quotes/:id/reject - public refus
router.post('/:id/reject', express.json(), express.urlencoded({ extended: true }), async (req, res, next) => {
    try {
        await ensureQuoteAuxSchemas();
        const { id } = req.params; const { token, reason } = req.body || {};
        if (!token) return res.status(400).send('Token manquant');
        const tokRes = await query(`SELECT * FROM quote_tokens WHERE quote_id = $1 AND token = $2 AND purpose = 'public'`, [id, token]);
        if (tokRes.rows.length === 0) return res.status(403).send('Token invalide');
        const t = tokRes.rows[0];
        ...
        await query(`UPDATE quotes SET status = 'rejected', updated_at = CURRENT_TIMESTAMP WHERE id = $1`, [id]);
        await query(`INSERT INTO quote_status_history (quote_id, event_type, ip, user_agent, metadata) VALUES ($1,'rejected',$2,$3,$4)`, [id, req.ip, req.get('user-agent') || null, JSON.stringify({ reason: reason || null })]);
        res.send('<html><body style="font-family:Arial;padding:20px"><h2>Décision enregistrée.</h2><p>Vous avez refusé ce devis.</p></body></html>');
    } catch (error) { next(error); }
});

// POST /api/quotes/:id/otp - demander l'envoi d'un OTP par e‑mail (public)
router.post('/:id/otp', express.json(), express.urlencoded({ extended: true }), async (req, res, next) => {
    try {
        await ensureQuoteAuxSchemas();
        const { id } = req.params; const { token } = req.body || {};
        if (!token) return res.status(400).send('Token manquant');
        const tokRes = await query(`SELECT * FROM quote_tokens WHERE quote_id = $1 AND token = $2 AND purpose = 'public'`, [id, token]);
        if (tokRes.rows.length === 0) return res.status(403).send('Token invalide');
        const t = tokRes.rows[0];
        ...
        const qRes = await query(`SELECT q.quote_number, c.email, c.first_name, c.last_name FROM quotes q JOIN clients c ON q.client_id = c.id WHERE q.id = $1`, [id]);
        if (qRes.rows.length === 0) return res.status(404).send('Devis non trouvé');
        const q = qRes.rows[0];
        if (!q.email) return res.status(400).send('Aucune adresse e‑mail du client enregistrée.');

        // Throttle: pas plus d'un envoi par 60s
        const recent = await query(`SELECT COUNT(*) AS c FROM quote_status_history WHERE quote_id = $1 AND event_type = 'otp_sent' AND created_at > NOW() - INTERVAL '60 seconds'`, [id]);
        if (parseInt(recent.rows[0].c || '0', 10) > 0) {
            return res.status(429).send('Veuillez patienter avant de demander un nouveau code.');
        }
        ...
        await query(`DELETE FROM quote_tokens WHERE quote_id = $1 AND purpose = 'otp'`, [id]);
        await query(`INSERT INTO quote_tokens (quote_id, token, purpose, expires_at) VALUES ($1,$2,'otp',$3)`, [id, `${salt}:${hash}`, expiresAt]);
        ...
        try { await transport.sendMail({ from, to: q.email, subject, html }); } catch (_) { /* ignore in dev stream */ }

        await query(`INSERT INTO quote_status_history (quote_id, event_type, metadata) VALUES ($1,'otp_sent',$2)`, [id, JSON.stringify({ to: q.email })]);

        res.send('<html><body style="font-family:Arial;padding:20px"><h2>Code envoyé.</h2><p>Un code à 6 chiffres a été envoyé par e‑mail. Entrez‑le pour accepter le devis.</p></body></html>');
    } catch (error) { next(error); }
});
