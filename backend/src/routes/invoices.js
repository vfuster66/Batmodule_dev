...
const paymentSchema = Joi.object({
    ...
    paymentMethod: Joi.string().valid('cash', 'check', 'transfer', 'card').default('cash'),
...
router.post('/', authenticateToken, async (req, res, next) => {
    try {
        ...
        const { clientId, quoteId, title, description, dueDate, notes, items } = value;
        ...
        const result = await transaction(async (client) => {
         ...
         RETURNING id, invoice_number, title, description, status, subtotal_ht, total_vat, total_ttc, paid_amount, due_date, notes, created_at, updated_at`,
                [req.user.userId, clientId, quoteId, invoiceNumber, title, description, calculations.subtotalHt, calculations.totalVat, calculations.totalTtc, dueDate, notes]
            );
           ...
           RETURNING id, service_id, description, quantity, unit_price_ht, unit_price_ttc, vat_rate, total_ht, total_ttc, sort_order, section_id`,
                    [invoice.id, item.serviceId, item.description, item.quantity, item.unitPriceHt, item.unitPriceTtc, item.vatRate, item.totalHt, item.totalTtc, item.sortOrder || i, item.sectionId || null]
                );
        ...
        // Audit
        try { await logAudit({ userId: req.user.userId, entityType: 'invoice', entityId: result.invoice.id, action: 'create', metadata: { invoiceNumber: result.invoice.invoice_number } }); } catch (_) { }
    ...
    try {
        ...
        if (paymentMethod === 'cash') {
            ...
            if (nf525Result.rows.length === 0) {
                return res.status(404).json({ error: 'Paramètres de l\'entreprise non trouvés' });
            }
            ...
            if (!settings.cash_payments_enabled) {
                return res.status(400).json({
                    ...
                    message: 'Les paiements en espèces sont désactivés pour cette entreprise'
                });
            }
            ...
            console.log(`[NF525] Paiement espèces: ${amount}€ - Facture: ${id} - Utilisateur: ${req.user.userId}`);
        }
        ...
        const result = await transaction(async (client) => {
            ...
            const newStatus = newPaidAmount >= parseFloat(invoice.total_ttc) ? 'paid' : 'pending';
    ...
    try {
        ...
        if (!['pending', 'paid', 'overdue'].includes(status)) {
            return res.status(400).json({
                ...
                message: 'Le statut doit être: pending, paid ou overdue (pas d\'annulation)'
            });
        }
       ...
       RETURNING id, status, updated_at`,
        ...
        // Audit
        try { await logAudit({ userId: req.user.userId, entityType: 'invoice', entityId: id, action: 'status', metadata: { status } }); } catch (_) { }
    ...
    try {
       ...
       ORDER BY sort_order, created_at`,
            ...
            try {
                ...
                for (const r of qiRes.rows) {
                    if (r && typeof r.sort_order === 'number') {
                        ...
                        if (r.discount_percent != null) discountBySortOrder.set(r.sort_order, parseFloat(r.discount_percent));
                        if (r.markup_percent != null) markupBySortOrder.set(r.sort_order, parseFloat(r.markup_percent));
                    }
                }
            } catch (_) { /* ignore */ }
        }
        ...
        const invoiceData = {
            ...
            sections: sections.map(s => ({ id: s.id, title: s.title, description: s.description, sortOrder: s.sort_order })),
            siteAddress,
            createdAt: invoice.created_at,
            updatedAt: invoice.updated_at
        };
        ...
        // Générer le PDF
        const pdfBuffer = await pdfService.generateInvoicePDF(invoiceData, companySettings);
...
router.post('/from-quote/:id', authenticateToken, async (req, res, next) => {
    ...
    try {
        ...
        const result = await transaction(async (client) => {
            ...
            // Calculer les totaux
            const calculations = calculationService.calculateTotals(itemsResult.rows.map(item => ({
                quantity: parseFloat(item.quantity),
                unitPriceHt: parseFloat(item.unit_price_ht),
                vatRate: parseFloat(item.vat_rate),
...
router.post('/:id/archive', authenticateToken, async (req, res) => {
    try {
        ...
        // Archiver la facture
        const archiveInfo = await archivingService.archiveInvoice(invoice, pdfBuffer, invoice);
...
router.get('/:id/verify', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Erreur lors de la vérification de la facture' });
    }
});
...
router.post('/advance', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Erreur lors de la création de la facture d\'acompte' });
    }
});
...
router.post('/final', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(500).json({ error: 'Erreur lors de la création de la facture de solde' });
    }
});
...
router.get('/:id/related', authenticateToken, async (req, res) => {
    try {
        ...
        const relatedInvoices = await advanceInvoiceService.getRelatedInvoices(req.user.userId, id);

        res.json(relatedInvoices);
    } catch (error) {
        console.error('Erreur lors de la récupération des factures liées:', error);
        res.status(500).json({ error: 'Erreur lors de la récupération des factures liées' });
    }
});
...
router.get('/:id/payment-status', authenticateToken, async (req, res) => {
    try {
        ...
        const paymentStatus = await advanceInvoiceService.getPaymentStatus(req.user.userId, id);

        res.json(paymentStatus);
    } catch (error) {
        console.error('Erreur lors de la récupération du statut de paiement:', error);
        res.status(500).json({ error: 'Erreur lors de la récupération du statut de paiement' });
    }
});
