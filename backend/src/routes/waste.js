...
const createWasteRecordSchema = Joi.object({
  ...
  transporterSiret: Joi.string()
    .pattern(/^\d{14}$/)
    .optional(),
  destinationFacility: Joi.string().max(255).optional(),
  facilitySiret: Joi.string()
    .pattern(/^\d{14}$/)
    .optional(),
...
router.get('/', authenticateToken, async (req, res) => {
  try {
    ...
    const wasteRecords = await wasteManagementService.getWasteRecords(
      req.user.userId,
      projectId
    )
...
router.get('/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params
    const wasteRecord = await wasteManagementService.getWasteRecordById(
      id,
      req.user.userId
    )
    ...
    if (error.message === 'Enregistrement de déchet non trouvé') {
      return res
        .status(404)
        .json({ error: 'Enregistrement de déchet non trouvé' })
    }
...
router.post('/', authenticateToken, async (req, res) => {
  try {
    ...
    const wasteRecord =
      await wasteManagementService.createWasteRecord(wasteData)
    ...
    res.status(500).json({
      error: "Erreur lors de la création de l'enregistrement de déchet",
    })
  }
})
...
router.put('/:id', authenticateToken, async (req, res) => {
  try {
    ...
    const wasteRecord = await wasteManagementService.updateWasteRecord(
      id,
      req.user.userId,
      value
    )
    ...
    res.status(500).json({
      error: "Erreur lors de la mise à jour de l'enregistrement de déchet",
    })
  }
})
...
router.delete('/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params
    const deleted = await wasteManagementService.deleteWasteRecord(
      id,
      req.user.userId
    )
    ...
    res.status(500).json({
      error: "Erreur lors de la suppression de l'enregistrement de déchet",
    })
  }
})
...
router.get('/compliance/check', authenticateToken, async (req, res) => {
  try {
    ...
    const report = await wasteManagementService.generateWasteReport(
      ...
      startDate
        ? new Date(startDate)
        : new Date(Date.now() - 365 * 24 * 60 * 60 * 1000), // Dernière année
