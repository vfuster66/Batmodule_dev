...
router.get('/export', authenticateToken, async (req, res, next) => {
    try {
        ...
        const headers = [
            ...
            'Est une entreprise', 'SIRET', 'N° TVA', 'Forme juridique', 'RCS', 'Code APE', 'Capital social', 'Notes'
        ];
...
router.post('/', authenticateToken, async (req, res, next) => {
    try {
        ...
        const { firstName, lastName, companyName, email, phone, addressLine1, addressLine2, postalCode, city, country, notes, isCompany, siret, vatNumber, legalForm, rcsNumber, apeCode, capitalSocial } = value;
       ...
       RETURNING id, first_name, last_name, company_name, email, phone, address_line1, address_line2, postal_code, city, country, notes, is_company, siret, vat_number, legal_form, rcs_number, ape_code, capital_social, created_at, updated_at`,
            [req.user.userId, firstName, lastName, companyName, email, phone, addressLine1, addressLine2, postalCode, city, country, notes, isCompany, siret, vatNumber, legalForm, rcsNumber, apeCode, capitalSocial]
        );
...
router.put('/:id', authenticateToken, async (req, res, next) => {
    try {
       ...
       RETURNING id, first_name, last_name, company_name, email, phone, address_line1, address_line2, postal_code, city, country, notes, is_company, siret, vat_number, legal_form, rcs_number, ape_code, capital_social, created_at, updated_at`,
            [firstName, lastName, companyName, email, phone, addressLine1, addressLine2, postalCode, city, country, notes, isCompany, siret, vatNumber, legalForm, rcsNumber, apeCode, capitalSocial, id, req.user.userId]
        );
        ...
        for (let i = 0; i < clients.length; i++) {
            ...
            try {
                ...
                const result = await query(
                    ...
                    [
                        req.user.userId, value.firstName, value.lastName, value.companyName,
                        value.email, value.phone, value.addressLine1, value.addressLine2,
                        value.postalCode, value.city, value.country, value.isCompany,
                        value.siret, value.vatNumber, value.legalForm, value.rcsNumber,
                        value.apeCode, value.capitalSocial, value.notes
                    ]
                );
