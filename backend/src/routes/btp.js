...
const reducedVATSchema = Joi.object({
  ...
  workType: Joi.string()
    .valid("renovation", "energy_improvement", "maintenance")
    .required(),
...
router.post("/validate-reverse-charge", authenticateToken, async (req, res) => {
  try {
    ...
    const validation =
      await btpValidationService.validateReverseChargeBTP(value);
...
router.post("/calculate-vat-rate", authenticateToken, async (req, res) => {
  try {
    ...
    const vatCalculation =
      await btpValidationService.calculateApplicableVATRate(value);
...
router.get("/info", authenticateToken, async (req, res) => {
  try {
    const btpInfo = {
      ...
      reducedVAT: {
        ...
        rates: {
          "10%": {
            description:
              "Travaux de rénovation dans des locaux à usage d'habitation",
            conditions: [
    ...
    console.error(
      "Erreur lors de la récupération des informations BTP:",
      error,
    );
    res
      .status(500)
