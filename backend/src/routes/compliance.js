...
router.get('/dashboard', authenticateToken, async (req, res) => {
  try {
    const dashboard = await complianceAlertService.getComplianceDashboard(
      req.user.userId
    )
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la récupération du tableau de bord' })
  }
})
...
router.post('/alerts/generate', authenticateToken, async (req, res) => {
  try {
    const result = await complianceAlertService.generateAllAlerts(
      req.user.userId
    )
...
router.put('/alerts/:id/resolve', authenticateToken, async (req, res) => {
  try {
    ...
    const alert = await complianceAlertService.resolveAlert(
      id,
      req.user.userId,
      value.notes
    )
...
router.delete('/alerts/:id', authenticateToken, async (req, res) => {
  try {
    const { id } = req.params
    const deleted = await complianceAlertService.deleteAlert(
      id,
      req.user.userId
    )
...
router.get('/expiring', authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: 'Erreur lors de la récupération des éléments expirants' })
  }
})
