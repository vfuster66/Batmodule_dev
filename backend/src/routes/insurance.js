...
const createInsuranceSchema = Joi.object({
  certificateType: Joi.string()
    .valid("decennale", "rc_pro", "garantie", "multirisque")
    .required(),
...
router.get("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération de l'assurance" });
  }
});
...
router.post("/", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la création de l'assurance" });
  }
});
...
router.put("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la mise à jour de l'assurance" });
  }
});
...
router.delete("/:id", authenticateToken, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Erreur lors de la suppression de l'assurance" });
  }
});
...
router.get("/alerts/expiring", authenticateToken, async (req, res) => {
  try {
    ...
    res.status(500).json({
      error: "Erreur lors de la récupération des assurances expirantes",
    });
  }
});
...
router.get("/compliance/check", authenticateToken, async (req, res) => {
  try {
    const compliance = await insuranceService.checkInsuranceCompliance(
      req.user.userId,
    );
...
router.get("/types/info", authenticateToken, async (req, res) => {
  try {
    ...
    console.error(
      "Erreur lors de la récupération des types d'assurances:",
      error,
    );
    res
      .status(500)
      .json({ error: "Erreur lors de la récupération des types d'assurances" });
  }
});
