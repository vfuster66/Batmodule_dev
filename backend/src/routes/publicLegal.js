...
router.get('/current', authenticateToken, async (req, res) => {
    try {
        const settings = await companySettingsService.getSettings(req.user.userId, false);
...
router.get('/mentions', async (req, res) => {
    try {
        ...
        const validCompanyId = uuidRegex.test(companyId) ? companyId : '8f843e79-7460-4f3e-874b-28ff8bab3316';

        const settings = await companySettingsService.getSettings(validCompanyId, false);
...
router.get('/privacy', async (req, res) => {
    try {
        ...
        res.status(500).json({
            error: 'Erreur lors de la récupération de la politique de confidentialité'
        });
    }
});
