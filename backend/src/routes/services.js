...
const serviceSchema = Joi.object({
  ...
  vat_rate: Joi.number().min(0).max(100).default(20.0),
...
router.get('/categories', authenticateToken, async (req, res, next) => {
  try {
    const result = await query(
      `SELECT id, name, description, color, category_number, created_at, updated_at
       FROM service_categories 
       WHERE user_id = $1
       ORDER BY category_number, name`,
      [req.user.userId]
    )
...
router.get('/', authenticateToken, async (req, res, next) => {
  try {
    ...
    const result = await query(
      `SELECT s.id, s.category_id, s.name, s.description, s.unit, s.price_ht, s.price_ttc, s.vat_rate, s.is_active, s.service_number, s.created_at, s.updated_at,
              sc.name as category_name, sc.color as category_color, sc.category_number
       FROM services s
    ...
    res.json({
      services: result.rows.map((service) => ({
        ...
        category_number: service.category_number,
        name: service.name,
