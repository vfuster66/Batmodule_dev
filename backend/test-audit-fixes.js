...
const pool = new Pool({
  connectionString:
    ...
    'postgres://batmodule:batmodule123@postgres:5432/batmodule',
})
...
async function testAuditFixes() {
  ...
  try {
    ...
    console.log(
      `   ✅ ${uniqueConstraints.rows.length} contraintes d'unicité trouvées`
    )
    ...
    console.log(
      `   ✅ ${btpColumns.rows.length} champs de validation BTP trouvés`
    )
    ...
    console.log(
      `   ✅ ${btpFunctions.rows.length} fonctions de validation BTP trouvées`
    )
    ...
    console.log(
      `   ✅ Table invoice_status_history avec ${historyTable.rows.length} colonnes`
    )
    ...
    console.log(
      `   ✅ ${archiveColumns.rows.length} colonnes d'archivage trouvées`
    )
    ...
    console.log(
      `   ✅ ${invoiceTypeCheck.rows.length} colonnes de types de factures trouvées`
    )
    ...
    if (
      ...
      console.log(
        '   ⚠️  JWT_SECRET utilise une valeur par défaut - À CHANGER EN PRODUCTION'
      )
    }
