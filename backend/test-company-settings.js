  ...
  try {
    ...
    if (loginResponse.data.token) {
      ...
      try {
        ...
        console.log(
          `📊 Données reçues: ${JSON.stringify(settingsResponse.data).substring(0, 100)}...`
        )
      } catch (error) {
        console.log(
      ...
      try {
        const complianceResponse = await axios.get(
          `${API_BASE}/company-settings/compliance-report`,
          {
            headers: { Authorization: `Bearer ${token}` },
          }
        )
        ...
        console.log(
          `📊 Score de conformité: ${complianceResponse.data.complianceScore || 'N/A'}`
        )
      } catch (error) {
        console.log(
          '❌ Erreur du rapport de conformité:',
          error.response?.data?.message || error.message
        )
      }
    } else {
